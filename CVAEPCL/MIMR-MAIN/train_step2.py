...
# Get the value of the observation well based on MIMR result
input_file1 = "./MIMR_results/MIMR_head_Pct0.999.hdf5"
f = h5py.File(input_file1, "r")
y_obs_head = np.array(f["y_obs"]).astype('int')
x_obs_head = np.array(f["x_obs"]).astype('int')
f.close()

input_file2 = "./MIMR_results/MIMR_conc_Pct0.9.hdf5"
f = h5py.File(input_file2, "r")
y_obs_conc = np.array(f["y_obs"]).astype('int')
x_obs_conc = np.array(f["x_obs"]).astype('int')
f.close()

head_gt_obs = add_noise(head_gt[:, y_obs_head, x_obs_head], device=torch_device)
conc_gt_obs = add_noise(conc_gt[:, :, y_obs_conc, x_obs_conc], device=torch_device)
...
for i in range(n_iter):
    ...
    head_pre = output_pre.Phi[:, y_obs_head, x_obs_head]
    Q_pre = output_pre.Q
    ...
    conc_pre = conc_pre[:, :, y_obs_conc, x_obs_conc]
