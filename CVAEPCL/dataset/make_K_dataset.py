...
# @Author  : Zhenjie Tang
# @File    : make_K_dataset.py
# @Description: generate the log-conductivity field datasets for WAE, the shape=[6, 41, 81]
...
kds = np.vstack(kds)
print(kds.shape)    # 27048 = 49 * 23 * 6 * 4
with open('./train_K_data/kds.pkl', 'wb') as file:
    pkl.dump(kds, file)

test_kds = []
for i in range(1, 5):
    test_im = scipy.io.loadmat(
        './test_K/K{}.mat'.format(i)
    )
...
print(test_kds.shape)   # 2208 = 4 * 23 * 6 * 4

with open('./test_K_data/test_kds.pkl', 'wb') as file:
